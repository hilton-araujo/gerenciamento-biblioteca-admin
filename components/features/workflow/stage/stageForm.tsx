...
const StageForm = ({ formik, isPending, isEditMode = false }: Props) => {
    return (
        <div>
            <form onSubmit={formik.handleSubmit}>
                <div className="w-full p-2 mt-2 rounded-md shadow-md bg-gray-50">
                    <fieldset className="h-full p-2 space-y-4 bg-white border rounded-lg shadow-md">
                        <legend className="text-lg font-medium ">Dados de Etapa</legend>
                        <div className="flex flex-col gap-2">
                            <TextField
                                {...generateFieldProps(formik, "designation", "Designação", "Digite a designação da etapa")}
                            />
                            <TextField {...generateFieldProps(formik, "stageOrder", "Ordem", "Digite a ordem da etapa", "number")} />
                        </div>
                    </fieldset>
                </div>
            </form>
